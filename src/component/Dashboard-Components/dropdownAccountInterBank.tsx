...
import { faCircleXmark, faAngleRight } from '@fortawesome/free-solid-svg-icons';
...
type ItemDropdownAccountInterBankProps = {
    title: string,
    items: Item[];
    selectedItem: Item;
...
const ItemDropdownAccountInterBank: React.FC<ItemDropdownAccountInterBankProps> = ({
    title,
    items,
    selectedItem,
    setSelectedItem,
    ...
    const filteredItems = items.filter(item =>
        ...
        item.attribute3.toLowerCase().includes(searchQuery.toLowerCase())
    );
                    ...
                    >
                        <div className="flex items-center hover:bg-[#ecf4e3] cursor-pointer rounded-2xl transition-all duration-300 ease-in-out p-2">
                            ...
                            <div>
                                ...
                                <p className="text-gray-600 text-xs font-bold">{item.attribute2}</p>
                                <p className="text-gray-600 text-xs font-medium">{item.attribute3}</p>
                            </div>
...
export default ItemDropdownAccountInterBank;